Shader "VolumeRendering/DirectVolumeRenderingShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Medical imaging enhancement parameters
            #define EDGE_ENHANCEMENT_STRENGTH 1.5
            #define FRACTURE_EDGE_THRESHOLD 0.8
            #define CONTRAST_ENHANCEMENT 1.3
            
            // Density-priority rendering parameters
            #define DENSITY_PRIORITY_STRENGTH 1.0
            #define BONE_PRIORITY 5
            #define VESSEL_PRIORITY 4
            #define SOFT_TISSUE_PRIORITY 3
            #define FAT_PRIORITY 2
            #define LUNG_PRIORITY 1
            #define AIR_PRIORITY 0
...
#include "Include/VolumeCutout.cginc"
            ...
            /// Density-priority raymarching helper functions
            
            /// Determine tissue type priority based on density value
            /// Higher density = higher priority (rendered first)
            int getTissuePriority(float density)
            {
                // Medical HU ranges mapped to priorities
                // Bone: 800+ HU (density > 0.53)
                if (density > 0.53)
                    return BONE_PRIORITY;
                
                // Vessel/Hemorrhage: 30-100 HU (density 0.22-0.33)
                if (density > 0.22 && density < 0.33)
                    return VESSEL_PRIORITY;
                
                // Soft tissue: -10 to 100 HU (density 0.30-0.33)
                if (density > 0.30 && density < 0.34)
                    return SOFT_TISSUE_PRIORITY;
                
                // Fat: -100 to -50 HU (density 0.15-0.20)
                if (density > 0.15 && density < 0.20)
                    return FAT_PRIORITY;
                
                // Lung: -500 to -100 HU (density 0.0-0.15)
                if (density > 0.0 && density < 0.15)
                    return LUNG_PRIORITY;
                
                // Air: < -500 HU (density < 0.0)
                return AIR_PRIORITY;
            }
            
            /// Calculate density weight factor for opacity suppression
            /// Higher density tissues suppress lower density contributions
            float calculateDensityWeight(float density, int priority)
            {
                ...
                return baseWeight * density;
            }
            
            /// Apply density-priority suppression to opacity
            /// Higher priority tissues suppress lower priority contributions
            float applyDensityPrioritySuppression(float opacity, float accumulatedAlpha, int priority, float density)
            {
                // Suppress lower priority contributions based on accumulated alpha and density weight
                float suppressionFactor = calculateDensityWeight(density, priority);
                float suppression = accumulatedAlpha * suppressionFactor * DENSITY_PRIORITY_STRENGTH;
                
                return opacity * (1.0 - suppression);
            }
            ...
            // Enhanced medical lighting with better edge detection
            float3 calculateMedicalLighting(float3 col, float3 normal, float3 lightDir, float3 eyeDir, 
                                           float specularIntensity, float density, float gradMag)
            {
                ...
                // Adaptive ambient based on gradient magnitude
                ...
                float3 r = normalize(reflect(-lightDir, normal));
                float rdotv = max(dot(r, v), 0.0);
                float specularPower = 32.0 + 32.0 * density;
            ...
            frag_out frag_dvr(frag_in i)
            {
                ...
                for (int iStep = 0; iStep < raymarchInfo.numSteps; iStep++)
                {
                    ...
                    // Apply density-priority suppression for medical-grade rendering
                    int tissuePriority = getTissuePriority(density);
                    src.a = applyDensityPrioritySuppression(src.a, col.a, tissuePriority, density);
#endif
                    ...
                    // Apply enhanced medical lighting
#if defined(LIGHTING_ON)
                    float factor = smoothstep(_LightingGradientThresholdStart, _LightingGradientThresholdEnd, gradMag);
                    
                    // Use enhanced medical lighting for better edge detection
                    float3 shaded = calculateMedicalLighting(src.rgb, gradient / gradMag, 
                                                            getLightDirection(-ray.direction), -ray.direction, 
                                                            0.3f, density, gradMag);
                    
                    // Edge enhancement for fracture detection
                    float edgeEnhance = edgeEnhancement(gradMag, FRACTURE_EDGE_THRESHOLD, 0.1);
                    shaded = lerp(shaded, src.rgb * 1.5, edgeEnhance * EDGE_ENHANCEMENT_STRENGTH);
                    ...
                    // Apply contrast enhancement for medical imaging
                    src.rgb = contrastEnhancement(src.rgb, CONTRAST_ENHANCEMENT);
